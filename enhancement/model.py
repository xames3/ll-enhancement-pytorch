...
class DecompositionCNN_Archive(BaseEnhancementCNN):
    """..."""
    ...
    def forward(self, x: _Tensor) -> t.Any:
        ...
        y = torch.sigmoid(output_features[:, 0:3, :, :])
        x = torch.sigmoid(output_features[:, 3:4, :, :])
        return y, x


class AdjustmentCNN_Archive(BaseEnhancementCNN):
    """..."""
    ...
    downsampling_layer_1: nn.Conv2d(
        in_channels=config.OUTPUT_CHANNELS,
        out_channels=config.OUTPUT_CHANNELS,
        kernel_size=config.KERNEL_SIZE,
        stride=2,
    ...
    downsampling_layer_2: nn.Conv2d(
        in_channels=config.OUTPUT_CHANNELS,
        out_channels=config.OUTPUT_CHANNELS,
        kernel_size=config.KERNEL_SIZE,
        stride=2,
    ...
    downsampling_layer_3: nn.Conv2d(
        in_channels=config.OUTPUT_CHANNELS,
        out_channels=config.OUTPUT_CHANNELS,
        kernel_size=config.KERNEL_SIZE,
        stride=2,
    ...
    upsampling_layer_1: nn.Conv2d(
        in_channels=config.OUTPUT_CHANNELS * 2,
        out_channels=config.OUTPUT_CHANNELS,
        kernel_size=config.KERNEL_SIZE,
        padding=1,
        padding_mode="replicate",
    )
    upsampling_layer_2: nn.Conv2d(
        in_channels=config.OUTPUT_CHANNELS * 2,
        out_channels=config.OUTPUT_CHANNELS,
        kernel_size=config.KERNEL_SIZE,
        padding=1,
    ...
    fusion_layer: nn.Conv2d(
        in_channels=config.OUTPUT_CHANNELS * 3,
        out_channels=config.OUTPUT_CHANNELS,
        kernel_size=1,
        padding=1,
        padding_mode="replicate",
    ...
    def forward(
        ...
        **kwargs: t.Any,
        ...
        """Implementation of forward pass."""
        input_img = torch.cat((y, x), dim=1)
        output_0 = self.base_layer(input_img)
        d_output_1 = self.relu(self.downsampling_layer_1(output_0))
        d_output_2 = self.relu(self.downsampling_layer_2(d_output_1))
        d_output_3 = self.relu(self.downsampling_layer_3(d_output_2))
        d_output = F.interpolate(
            d_output_3, (d_output_2.size()[2], d_output_2.size()[3])
        )
        u_output_1 = self.relu(
            self.upsampling_layer_1(torch.cat((d_output, d_output_2), dim=1))
        )
        _u_output_1 = F.interpolate(
            u_output_1, (d_output_1.size()[2], d_output_1.size()[3])
        )
        u_output_2 = self.relu(
            self.upsampling_layer_2(
                torch.cat((_u_output_1, d_output_1), dim=1)
            )
        )
        _u_output_2 = F.interpolate(
            u_output_2, (output_0.size()[2], output_0.size()[3])
        )
        u_output_3 = self.relu(
            self.upsampling_layer_3(torch.cat((_u_output_2, output_0), dim=1))
        )
        u_y_1 = F.interpolate(u_output_1, size=(y.size()[2], y.size()[3]))
        u_y_2 = F.interpolate(u_output_2, size=(y.size()[2], y.size()[3]))
        combined_features = torch.cat((u_y_1, u_y_2, u_output_3), dim=1)
...
class Adjustment(nn.Module):
    ...
    def forward(self, input_L, input_R):
        ...
        deconv1 = self.relu(
            self.net2_deconv1_1(torch.cat((out3_up, out2), dim=1))
        )
        deconv1_up = F.interpolate(
            deconv1, size=(out1.size()[2], out1.size()[3])
        )
        deconv2 = self.relu(
            self.net2_deconv1_2(torch.cat((deconv1_up, out1), dim=1))
        )
        deconv2_up = F.interpolate(
            deconv2, size=(out0.size()[2], out0.size()[3])
        )
        deconv3 = self.relu(
            self.net2_deconv1_3(torch.cat((deconv2_up, out0), dim=1))
        )

        deconv1_rs = F.interpolate(
            deconv1, size=(input_R.size()[2], input_R.size()[3])
        )
        deconv2_rs = F.interpolate(
            deconv2, size=(input_R.size()[2], input_R.size()[3])
        )
...
class EnhanceCNN(nn.Module):
    def __init__(self):
        super(EnhanceCNN, self).__init__()

        self.decomposition = Decomposition()
        self.adjustment = Adjustment()

    def forward(self, input_low, input_high):
        # Forward decompositionpNet
        ...
        R_low, I_low = self.decomposition(input_low)
        R_high, I_high = self.decomposition(input_high)

        # Forward Adjustment
        I_delta = self.adjustment(I_low, R_low)
        ...
        self.loss_adjustment = self.relight_loss + 3 * self.Ismooth_loss_delta
    ...
    def gradient(self, input_tensor, direction):
        self.smooth_kernel_x = torch.FloatTensor([[0, 0], [-1, 1]]).view(
            (1, 1, 2, 2)
        )
    ...
    def evaluate(self, epoch_num, eval_low_data_names, vis_dir, train_phase):
        print(
            "Evaluating for phase %s / epoch %d..." % (train_phase, epoch_num)
        )
        ...
        for idx in range(len(eval_low_data_names)):
            ...
            filepath = os.path.join(
                vis_dir,
                "eval_%s_%d_%d.png" % (train_phase, idx + 1, epoch_num),
            )
    ...
    def save(self, iter_num, ckpt_dir):
        ...
        if self.train_phase == "decomposition":
            torch.save(self.decomposition.state_dict(), save_name)
        elif self.train_phase == "adjustment":
            torch.save(self.adjustment.state_dict(), save_name)
    ...
    def load(self, ckpt_dir):
        ...
        if os.path.exists(load_dir):
            ...
            if len(load_ckpts) > 0:
                ...
                if self.train_phase == "decomposition":
                    self.decomposition.load_state_dict(ckpt_dict)
                elif self.train_phase == "adjustment":
                    self.adjustment.load_state_dict(ckpt_dict)
    ...
    def train(
        ...
        self.train_op_decomposition = optim.Adam(
            self.decomposition.parameters(), lr=lr[0], betas=(0.9, 0.999)
        )
        self.train_op_adjustment = optim.Adam(
            self.adjustment.parameters(), lr=lr[0], betas=(0.9, 0.999)
        )
        ...
        print(
            "Start training for phase %s, with start epoch %d start iter %d : "
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for param_group in self.train_op_decomposition.param_groups:
                param_group["lr"] = self.lr
            for param_group in self.train_op_adjustment.param_groups:
                param_group["lr"] = self.lr
            for batch_id in range(start_step, numBatch):
                ...
                for patch_id in range(batch_size):
                    ...
                    train_low_img = (
                        np.array(train_low_img, dtype="float32") / 255.0
                    )
                    train_high_img = Image.open(
                        train_high_data_names[image_id]
                    )
                    train_high_img = (
                        np.array(train_high_img, dtype="float32") / 255.0
                    )
                    # Take random crops
                    h, w, _ = train_low_img.shape
                    x = random.randint(0, h - patch_size)
                    y = random.randint(0, w - patch_size)
                    train_low_img = train_low_img[
                        x : x + patch_size, y : y + patch_size, :
                    ]
                    train_high_img = train_high_img[
                        x : x + patch_size, y : y + patch_size, :
                    ]
                    # Data augmentation
                ...
                if self.train_phase == "decomposition":
                    self.train_op_decomposition.zero_grad()
                    self.loss_decomposition.backward()
                    self.train_op_decomposition.step()
                    loss = self.loss_decomposition.item()
                elif self.train_phase == "adjustment":
                    self.train_op_adjustment.zero_grad()
                    self.loss_adjustment.backward()
                    self.train_op_adjustment.step()
                    loss = self.loss_adjustment.item()
    ...
    def predict(
        self, frames, webcam, ckpt_dir=config.PROJECT_PATHS["../models"]
    ):
        # Load the network with a pre-trained checkpoint
        self.train_phase = "decomposition"
        ...
        if load_model_status:
            ...
            raise Exception
        save_R_L = False
        test_low_img = np.array(frames, dtype="float32") / 255.0
        ...
        cat_image = np.transpose(cat_image, (1, 2, 0))
        im = np.clip(cat_image * 255.0, 0, 255.0).astype("uint8")
        return im
